...
authorizer.keepConnectionOpen = () => {
   ...
   authorizer.mconn = mysql.createConnection(config.db.databaseTwo);
...
authorizer.getUser = (authHeader, callback) => {
   ...
   authorizer.mconn.query(`SELECT * FROM databaseOne WHERE key=?`, [authorizer.apiKey], (error, result, fields) => {
